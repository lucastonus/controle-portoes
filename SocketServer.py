...
class SocketServer:
			...
			if (payload != None and 'gate' in payload):
				...
				if (payload_gate in [self.GATE_OUTSIDE, self.GATE_INSIDE, self.GATE_BOTH]):
					...
					if (result[1] == ResponseType.HTTP_CODE['OK']):
						DBConn().insert('INSERT INTO log (id_user, gate) VALUES (?, ?)', [id_user, gate])
	...
	def logs(self):
		logs = DBConn().select('SELECT l.id, u.name, l.gate, strftime("%d/%m/%Y %H:%M:%S", datetime(l.time, "localtime")) FROM log l JOIN user u ON u.id = l.id_user ORDER BY l.id DESC LIMIT 10', [])
		responseData = []
		for log in logs:
			responseData.append({
				'id': log[0],
